...
public final class Http2ExtendedConnectProtocol implements WebSocketProtocolStrategy {
    ...
    private static final class H2WebSocketExchangeHandler implements AsyncClientExchangeHandler {
        ...
        private final AtomicLong assemblingBufSize = new AtomicLong();
            ...
            switch (op) {
                ...
                case FrameOpcode.BINARY:
                    ...
                    if (!fin) {
                        ...
                        assemblingBuf = ByteBuffer.allocate(Math.max(1024, payload.remaining()));
                        assemblingBufSize.set(0);
                        try {
                ...
                case FrameOpcode.CONT:
                    ...
                    if (fin) {
                        ...
                        final ByteBuffer full = ByteBuffer.wrap(body);
        ...
        private void deliverSingle(final int opcode, final ByteBuffer payload, final boolean rsv1) {
            ...
            if (rsv1 && decChain != null) {
                try {
                    final byte[] decoded = decChain.decode(WebSocketBufferOps.toBytes(data));
        ...
        private String decodeTextStrict(final ByteBuffer payload) throws WebSocketProtocolException {
            utf8Decoder.reset();
            try {
                return utf8Decoder.decode(payload.asReadOnlyBuffer()).toString();
        ...
        private final class H2WebSocket implements WebSocket {
            ...
            public boolean sendBinaryBatch(final List<ByteBuffer> fragments, final boolean finalFragment) {
                ...
                for (final ByteBuffer b : fragments) {
                    if (b != null) {
                        merged.put(b.asReadOnlyBuffer());
                    }
                }
                merged.flip();
                return sendBinary(merged, finalFragment);
            }
            ...
            private boolean enqueueDataFrame(final int opcode, final ByteBuffer payload, final boolean fin, final boolean firstFragment) {
                ...
                byte[] out = WebSocketBufferOps.toBytes(payload);
                int rsv = 0;
                if (encChain != null) {
