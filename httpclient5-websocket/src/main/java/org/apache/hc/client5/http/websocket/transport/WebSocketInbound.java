...
final class WebSocketInbound {
    ...
    void onInputReady(final IOSession ioSession, final ByteBuffer src) {
        try {
            ...
            for (; ; ) {
                ...
                if (s.closeReceived.get() && op != FrameOpcode.CLOSE) {
                    continue;
                }
    ...
    private void appendToInbuf(final ByteBuffer src) {
        ...
        s.inbuf = WebSocketBufferOps.ensureCapacity(s.inbuf, src.remaining());
        s.inbuf.put(src);
    }
    ...
    private void startMessage(final int opcode, final ByteBuffer payload, final boolean rsv1, final IOSession ioSession) {
        ...
        s.assemblingBuf = ByteBuffer.allocate(initial);
        s.assemblingSize = 0L;
        appendToMessage(payload, ioSession);
    }
    ...
    private void appendToMessage(final ByteBuffer payload, final IOSession ioSession) {
        ...
        s.assemblingBuf = WebSocketBufferOps.ensureCapacity(s.assemblingBuf, n);
        s.assemblingBuf.put(payload.asReadOnlyBuffer());
    }

    private void deliverAssembledMessage() {
        s.assemblingBuf.flip();
        final byte[] body = new byte[s.assemblingBuf.remaining()];
        s.assemblingBuf.get(body);
