...
class WebSocketServerProcessor {
            ...
            switch (type) {
                ...
                case BINARY:
                    if (frame.isFin()) {
                        ...
                        continuationBuf = startContinuation(type, frame.getPayload());
                        continuationType = type;
                    }
                    break;
                case CONTINUATION:
                    if (continuationBuf == null || continuationType == null) {
                        throw new WebSocketException("Unexpected continuation frame");
                    }
                    continuationBuf = appendContinuation(continuationBuf, frame.getPayload());
                    if (frame.isFin()) {
                        continuationBuf.flip();
                        dispatchMessage(continuationType, continuationBuf);
    ...
    private ByteBuffer startContinuation(final WebSocketFrameType type, final ByteBuffer payload) throws WebSocketException {
        if (payload.remaining() > maxMessageSize) {
            throw new WebSocketProtocolException(1009, "Message too large: " + payload.remaining());
        }
        final ByteBuffer buf = ByteBuffer.allocate(Math.max(1024, payload.remaining()));
        return appendContinuation(buf, payload);
    }

    private ByteBuffer appendContinuation(final ByteBuffer inputBuf, final ByteBuffer payload) throws WebSocketException {
        final int n = payload.remaining();
        if (inputBuf.position() + n > maxMessageSize) {
            throw new WebSocketProtocolException(1009, "Message too large: " + (inputBuf.position() + n));
        }
        final ByteBuffer buf = WebSocketBufferOps.ensureCapacity(inputBuf, n);
        buf.put(payload.asReadOnlyBuffer());
        return buf;
    }
