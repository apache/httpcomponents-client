 ...
 * store retains at most {@code maxEntries} dictionaries and {@code maxBytes} bytes of dictionary
 * content; once either bound is exceeded the eldest entries are evicted in insertion order until
 * the store is back within capacity. Adding a
...
public final class BasicCompressionDictionaryStore implements CompressionDictionaryStore {
    ...
    /**
     * Creates a store bounded to the given number of dictionaries and total content size.
     *
     * @param maxEntries the maximum number of dictionaries to retain; must be positive.
     * @param maxBytes   the maximum total dictionary content size; must be positive.
    ...
    public BasicCompressionDictionaryStore(
        ...
        this.maxBytes = Args.positive(maxBytes, "Maximum bytes");
     ...
     * evicts its eldest entries until neither configured bound is exceeded. A dictionary larger
     * than the total byte bound cannot fit and is ignored.
     *
    ...
    public void add(
        ...
        if (dictionary.getContentLength() > maxBytes) {
            return;
        }
        ...
        try {
            ...
            final CompressionDictionary previous = dictionaries.remove(key);
            if (previous != null) {
                currentBytes -= previous.getContentLength();
            }
            dictionaries.put(key, dictionary);
            currentBytes += dictionary.getContentLength();

            while (dictionaries.size() > maxEntries || currentBytes > maxBytes) {
                final Iterator<Map.Entry<Key, CompressionDictionary>> iterator =
                        dictionaries.entrySet().iterator();
                if (iterator.hasNext()) {
                    currentBytes -= iterator.next().getValue().getContentLength();
                    iterator.remove();
                }
            }
        } finally {
    ...
    public void clear(final CookieStore partition) {
        ...
        try {
            ...
            while (iterator.hasNext()) {
                final Map.Entry<Key, CompressionDictionary> entry = iterator.next();
                if (entry.getKey().partition == partition) {
                    currentBytes -= entry.getValue().getContentLength();
                    iterator.remove();
                }
            }
        } finally {
