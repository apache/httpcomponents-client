...
public final class DefaultCompressionDictionaryUrlPatternMatcher
    ...
    private static final class ConstructorParts {
        ...
        static ConstructorParts parse(final String input, final URI baseUri) {
            if (input == null || baseUri == null || !baseUri.isAbsolute()) {
                throw new IllegalArgumentException("Invalid URL pattern");
            }
            final String constructorInput = input.trim().isEmpty() ? "" : input;
            rejectRegexpGroups(constructorInput);

            final ConstructorParts result = new ConstructorParts();
            final int authorityMarker = findAuthorityMarker(constructorInput);
            if (authorityMarker >= 0) {
                result.parseAbsolute(constructorInput, authorityMarker);
            } else if (constructorInput.startsWith("//")) {
                result.protocol = baseUri.getScheme().toLowerCase(Locale.ROOT);
                result.parseAuthorityAndTail(constructorInput, 2);
            } else {
                result.protocol = baseUri.getScheme().toLowerCase(Locale.ROOT);
                result.hostname = normalizeHost(baseUri.getHost());
                result.port = normalizedPort(baseUri);
                result.parseRelativeTail(constructorInput, baseUri);
            }
            return result;
        }
        ...
        private void parseAuthorityAndTail(
            ...
            if (bracedSlash >= 0) {
                parseBracedAuthorityAndTail(input, authorityStart, bracedSlash);
                return;
            }
        ...
        private void parseBracedAuthorityAndTail(
                final String input,
                final int authorityStart,
                final int bracedSlash) {
            final int openBrace = findBracedGroupStart(input, authorityStart, bracedSlash);
            if (openBrace < 0) {
                throw new IllegalArgumentException("Invalid URL pattern group");
            }
            final int closeBrace = findClosingBrace(input, bracedSlash + 1);
            if (closeBrace < 0) {
                throw new IllegalArgumentException("Unterminated URL pattern group");
            }
            if (closeBrace + 1 < input.length()
                    && isPatternModifier(input.charAt(closeBrace + 1))) {
                throw new IllegalArgumentException(
                        "Modified groups cannot cross URL components");
            }

            final String authority = input.substring(authorityStart, openBrace)
                    + input.substring(openBrace + 1, bracedSlash);
            final String tail = input.substring(bracedSlash, closeBrace)
                    + input.substring(closeBrace + 1);
            parseAuthority(authority);
            parseTail(tail, null);
        }
        ...
        private static int findBracedGroupStart(
                final String value,
                final int start,
            ...
            int openBrace = -1;
            boolean escaped = false;
            for (int i = start; i < delimiterIndex; i++) {
                final char ch = value.charAt(i);
                if (escaped) {
                    escaped = false;
                } else if (ch == '\\') {
                    ...
                    if (depth > 1) {
                        throw new IllegalArgumentException(
                                "Nested groups cannot cross URL components");
                    }
                    openBrace = i;
                } else if (ch == '}') {
                    if (depth == 0) {
                        throw new IllegalArgumentException(
                                "Unexpected URL pattern group terminator");
                    }
                    depth--;
                    if (depth == 0) {
                        openBrace = -1;
        ...
        private static boolean isPatternModifier(final char ch) {
            return ch == '?' || ch == '*' || ch == '+';
        }
        ...
        private void parseRelativeTail(final String input, final URI baseUri) {
            if (input.isEmpty()) {
                pathname = resolvePath(input, baseUri);
        ...
        private static String resolvePath(
            ...
            final String beforeHash = hashIndex >= 0
                    ? input.substring(0, hashIndex)
                    : input;
            final int queryIndex = findQueryDelimiter(beforeHash);
            final String path = queryIndex >= 0
                    ? beforeHash.substring(0, queryIndex)
                    : beforeHash;
    ...
    private static final class ComponentCompiler {
        ...
        private void appendLiteral(
            ...
            if (literal.length() > 0) {
                final String value = component == Component.PATHNAME
                        ? canonicalizePathLiteral(literal.toString())
                        : literal.toString();
                result.append(Pattern.quote(value));
                literal.setLength(0);
            }
        }

        private static String canonicalizePathLiteral(final String value) {
    ...
    private static String rawPath(final URI uri) {
        ...
        return value == null || value.isEmpty() ? "/" : normalizePath(value);
    }
