 ...
 * downstream consumer accepted is accumulated into an in-memory buffer. After the downstream
 * accepts the end of stream, the buffered bytes are handed to the
 * {@link CompressionDictionaryStore} as a new
...
final class DictionaryCapturingAsyncDataConsumer implements AsyncDataConsumer {
    ...
    /**
     * Registers an intact supported dictionary before signalling stream end downstream. Publishing
     * first guarantees that callbacks triggered by downstream completion can immediately negotiate
     * the dictionary on a subsequent request.
     *
