...
public final class Rfc6724AddressSelectingDnsResolver implements DnsResolver {
    ...
    // --- RFC 6724 helpers (minimal port of the algo you used in the operator) ---
    ...
    private static List<InetAddress> applyFamilyPreference(
        ...
        switch (pref) {
            ...
            case INTERLEAVE:
            default: {
                final List<InetAddress> v6 = new LinkedList<>();
                final List<InetAddress> v4 = new LinkedList<>();
                for (final InetAddress a : rfcSorted) {
                    if (a instanceof Inet6Address) {
                        v6.add(a);
                    } else {
                ...
                while (!v6.isEmpty() || !v4.isEmpty()) {
                    if (startV6) {
                        if (!v6.isEmpty()) {
                            merged.add(v6.remove(0));
                        }
                        if (!v4.isEmpty()) {
                            merged.add(v4.remove(0));
                        }
                    } else {
                        if (!v4.isEmpty()) {
                            merged.add(v4.remove(0));
    ...
    private static final Comparator<Info> RFC6724_COMPARATOR = (a, b) -> {
        final InetAddress DA = a.dst, DB = b.dst;
        final InetAddress SourceDA = a.src, SourceDB = b.src;
        final Attr attrDA = a.dstAttr, attrDB = b.dstAttr;
        final Attr attrSourceDA = a.srcAttr, attrSourceDB = b.srcAttr;
    ...
    private static String addr(final InetAddress a) {
        ...
        final boolean v6 = a instanceof Inet6Address;
        return v6 ? "IPv6" : "IPv4" + "(" + a.getHostAddress() + ")";
    }

    private static List<String> fmt(final InetAddress[] arr) {
        final List<String> out = new ArrayList<>(arr.length);
        for (final InetAddress a : arr) out.add(addr(a));
        return out;
    }
