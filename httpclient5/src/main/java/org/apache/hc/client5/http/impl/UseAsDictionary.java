 ...
 */
@Internal
public final class UseAsDictionary {
    ...
    /**
     * Parses the {@code Use-As-Dictionary} offer from {@code headers} as an HTTP Structured Field
     * Dictionary, or returns {@code null} when no such header is present so the caller does not need
     * a separate presence check. Field values are read in place from each header line rather than
     * copied into a combined string, and members split across several field lines accumulate into one
     * offer. Members other than {@code match}, {@code match-dest}, {@code id} and {@code type} are
     * accepted and ignored, as are member parameters, so unknown extensions do not break parsing. A
     * member whose value has the wrong Structured Field type for its key is rejected. The
     * {@code match} member is mandatory; the others fall back to their defaults when absent.
     *
     * @param headers the message headers to read the offer from.
     * @param name    the header name to parse.
     * @return the parsed offer, or {@code null} if no {@code name} header is present.
     * @throws ProtocolException if a header is present but malformed against the Structured Field
     *                           grammar, if {@code match} is missing or not a String, if
     *                           {@code match-dest} is not an inner list of Strings, if {@code id} is
     *                           not a String of at most 1024 characters, or if {@code type} is not a
     *                           Token.
     */
    public static UseAsDictionary parse(final MessageHeaders headers, final String name) throws ProtocolException {
        final Members members = new Members();
        MessageSupport.parseHeadersStrict(headers, name, (value, cursor) -> {
            members.present = true;
            parseMembers(value, cursor, members);
        });
        return members.present ? members.build() : null;
    }

    /**
     * Parses a single {@code Use-As-Dictionary} header value as an HTTP Structured Field Dictionary.
     *
     ...
     *                        {@code match} is missing or not a String, if {@code match-dest} is not an
     *                        inner list of Strings, if {@code id} is not a String of at most 1024
     *                        characters, or if {@code type} is not a Token.
     */
    public static UseAsDictionary parse(final String value) throws ParseException {
        Args.notBlank(value, "Use-As-Dictionary");
        final Members members = new Members();
        parseMembers(value, new ParserCursor(0, value.length()), members);
        return members.build();
    }

    /**
     * Parses the Dictionary members between the cursor position and its upper bound into
     * {@code members}, so that members carried on several field lines combine into one offer.
     *
     * @throws ParseException if the segment is not ASCII or violates the Structured Field grammar.
     */
    private static void parseMembers(
            final CharSequence value,
            final ParserCursor cursor,
            final Members members) throws ParseException {
        final int start = cursor.getPos();
        final int end = cursor.getUpperBound();
        for (int i = start; i < end; i++) {
            if (value.charAt(i) > 0x7f) {
                throw new ParseException("Structured Field value is not ASCII");
            }
        }
        final Parser parser = new Parser(value, start, end);
        parser.skipSp();
        while (!parser.atEnd()) {
            ...
            if (!parser.consume(',')) {
                throw new ParseException(
                        "Invalid Use-As-Dictionary dictionary separator");
            }
            parser.skipOws();
            if (parser.atEnd()) {
                throw new ParseException(
                        "Trailing comma in Use-As-Dictionary");
            }
        }
        cursor.updatePos(end);
    }

    /**
     * Accumulates the recognised Dictionary members as the field is read, then validates them into an
     * immutable {@link UseAsDictionary}. A repeated member takes its last value, as required for a
    ...
    private static final class Members {
        ...
        private Value match;
        private Value matchDest;
        private Value id;
        private Value type;

        void put(final String key, final Value member) {
            switch (key) {
                case "match":
                    match = member;
                    break;
        ...
        UseAsDictionary build() throws ParseException {
            if (match == null) {
                throw new ParseException(
                        "Use-As-Dictionary requires a match member");
            }
            if (match.kind != Value.STRING) {
                throw new ParseException(
                        "Invalid Use-As-Dictionary match member");
            }
            if (matchDest != null
                    && (matchDest.kind != Value.INNER_LIST || !matchDest.stringList)) {
                throw new ParseException(
                        "Invalid Use-As-Dictionary match-dest member");
            }
            if (id != null
                    && (id.kind != Value.STRING || id.stringValue.length() > 1024)) {
                throw new ParseException(
                        "Invalid Use-As-Dictionary id member");
            }
            if (type != null && type.kind != Value.TOKEN) {
                throw new ParseException(
                        "Invalid Use-As-Dictionary type member");
            }
            return new UseAsDictionary(
                    match.stringValue,
                    matchDest != null ? matchDest.listValue : Collections.<String>emptyList(),
                    id != null ? id.stringValue : "",
                    type != null ? type.stringValue : RAW);
        }
    }
    ...
    private static final class Parser {
        ...
        Parser(final CharSequence value, final int start, final int end) {
            this.value = value;
            this.pos = start;
            this.end = end;
        }
        ...
        String parseKey() throws ParseException {
            ...
            return value.subSequence(start, pos).toString();
        }
        ...
        void parseByteSequence() throws ParseException {
            ...
            final String encoded = value.subSequence(start, pos).toString();
            pos++;
            try {
