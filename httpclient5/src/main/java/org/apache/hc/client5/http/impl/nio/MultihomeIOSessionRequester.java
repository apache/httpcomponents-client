...
final class MultihomeIOSessionRequester {
    ...
    private void runStaggered(
        ...
        // Apply family preference (filter-only for *_ONLY)
        ...
        // - INTERLEAVE/DEFAULT -> start with family of first resolver address (keeps RFC6724 order)
        ...
        // ---- HEv2 schedule: two independent clocks ----
        final List<InetSocketAddress> A = startWithV6 ? v6Try : v4Try; // starting family
        final List<InetSocketAddress> B = startWithV6 ? v4Try : v6Try; // other family

        long tA = 0L;
        long tB = A.isEmpty() ? 0L : otherFamilyDelayMs; // if A empty, start B at t=0

        int iA = 0, iB = 0;

        // Kick off first attempts (if present)
        if (iA < A.size()) {
        ...
        // Continue interleaving in time (A at 0,Δ,2Δ… ; B at δ,δ+Δ,δ+2Δ…)
