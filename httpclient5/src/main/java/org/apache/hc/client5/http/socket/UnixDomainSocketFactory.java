...
public final class UnixDomainSocketFactory {
    ...
    public Socket createSocket() throws IOException {
        ...
        try {
            if (IMPLEMENTATION == Implementation.JDK) {
                // JDK16 only supports UDS through the SocketChannel API, but the sync client is coupled
                // to the legacy Socket API. In order to use JDK sockets, we first need to write an
