...
class TestHttpRoute {
    ...
    void testUnixDomainSocketModeling() {
        final Path uds1 = (new File("/var/run/docker.sock")).toPath();
        ...
        final Path uds2 = (new File("/var/run/docker.sock")).toPath();
    ...
    void testUnixDomainSocketValidation() {
        final Path uds = (new File("/var/run/docker.sock")).toPath();
