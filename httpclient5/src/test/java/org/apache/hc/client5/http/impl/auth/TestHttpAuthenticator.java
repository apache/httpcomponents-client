...
public class TestHttpAuthenticator {
    ...
    abstract class CacheableAuthState implements AuthScheme {
        ...
        public String getName() {
            return StandardAuthScheme.BASIC.id;
        }
    ...
    public void setUp() throws Exception {
        ...
        this.authSchemeRegistry = RegistryBuilder.<AuthSchemeFactory>create()
            ...
            .register(StandardAuthScheme.NTLM.id, NTLMSchemeFactory.INSTANCE).build();
    ...
    public void testUpdateAuthExchange() throws Exception {
        ...
        response.setHeader(HttpHeaders.WWW_AUTHENTICATE, StandardAuthScheme.BASIC.id + " realm=test");
    ...
    public void testAuthentication() throws Exception {
        ...
        final HttpResponse response = new BasicHttpResponse(HttpStatus.SC_UNAUTHORIZED, "UNAUTHORIZED");
        response.addHeader(new BasicHeader(HttpHeaders.WWW_AUTHENTICATE, StandardAuthScheme.BASIC.id + " realm=\"test\""));
        response.addHeader(new BasicHeader(HttpHeaders.WWW_AUTHENTICATE, StandardAuthScheme.DIGEST.id + " realm=\"realm1\", nonce=\"1234\""));
        ...
        Assert.assertEquals(StandardAuthScheme.DIGEST.id, authScheme1.getName());
        final AuthScheme authScheme2 = options.poll();
        Assert.assertNotNull(authScheme2);
        Assert.assertEquals(StandardAuthScheme.BASIC.id, authScheme2.getName());
    ...
    public void testAuthenticationCredentialsForBasic() throws Exception {
        ...
        final Credentials credentials = new UsernamePasswordCredentials("user", "pass".toCharArray());
        Mockito.when(this.credentialsProvider.getCredentials(Mockito.eq(new AuthScope(host, "test", StandardAuthScheme.BASIC.id)),
                                                             Mockito.<HttpContext>any())).thenReturn(credentials);
        ...
        final AuthScheme authScheme1 = options.poll();
        Assert.assertNotNull(authScheme1);
        Assert.assertEquals(StandardAuthScheme.BASIC.id, authScheme1.getName());
        Assert.assertNull(options.poll());
    }
    ...
    public void testAuthenticationHandshaking() throws Exception {
        ...
        final HttpResponse response = new BasicHttpResponse(HttpStatus.SC_UNAUTHORIZED, "UNAUTHORIZED");
        response.addHeader(new BasicHeader(HttpHeaders.WWW_AUTHENTICATE, StandardAuthScheme.BASIC.id + " realm=\"test\""));
        response.addHeader(new BasicHeader(HttpHeaders.WWW_AUTHENTICATE, StandardAuthScheme.DIGEST.id + " realm=\"realm1\", stale=true, nonce=\"1234\""));
        response.addHeader(new BasicHeader(HttpHeaders.WWW_AUTHENTICATE, "whatever realm=\"realm1\", stuff=\"1234\""));
    ...
    public void testAuthenticationNoMatchingChallenge() throws Exception {
        ...
        final Credentials credentials = new UsernamePasswordCredentials("user", "pass".toCharArray());
        Mockito.when(this.credentialsProvider.getCredentials(Mockito.eq(new AuthScope(host, "realm1", StandardAuthScheme.DIGEST.id)),
                                                             Mockito.<HttpContext>any())).thenReturn(credentials);
