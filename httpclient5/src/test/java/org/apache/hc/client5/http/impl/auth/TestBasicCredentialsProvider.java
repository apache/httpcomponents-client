...
public class TestBasicCredentialsProvider {
    ...
    public void testCredentialsMatching() {
        ...
        Credentials got = state.getCredentials(new AuthScope("http", "someotherhost", 80, "someotherrealm", StandardAuthScheme.BASIC.id), null);
        Credentials expected = creds1;
        Assert.assertEquals(expected, got);

        got = state.getCredentials(new AuthScope("http", "someotherhost", 80, "somerealm", StandardAuthScheme.BASIC.id), null);
        expected = creds2;
        Assert.assertEquals(expected, got);

        got = state.getCredentials(new AuthScope("http", "somehost", 80, "someotherrealm", StandardAuthScheme.BASIC.id), null);
        expected = creds3;
        Assert.assertEquals(expected, got);
    }
